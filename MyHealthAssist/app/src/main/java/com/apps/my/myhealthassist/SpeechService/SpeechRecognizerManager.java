...
public class SpeechRecognizerManager {
    ...
    private static int ListeningCounter = 1;
    ...
    public SpeechRecognizerManager(Context context, onResultsReady listener)
    {
        ...
        mSpeechRecognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 1500);
        mSpeechRecognizerIntent.putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 1500);
    ...
    {
        if(Utility.isListening()) {
            Utility.setListening(false);
    ...
    protected class SpeechRecognitionListener implements RecognitionListener
    {
        ...
        public synchronized void onError(int error)
        {
            ...
            Utility.getListeningLatch().countDown();
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        if (Utility.getMessageLatch() != null)
                            Utility.getMessageLatch().await();
    ...
    private void startListening()
    {
        if(!Utility.isListening())
        {
            Utility.setListening(true);
            Utility.setListeningLatch(1);

            Utility.ListeningCounter++;

            if (Utility.ListeningCounter > ListeningCounter) {
                Utility.setSilentMode(true);
            }
            ...
            Log.d(TAG, "INFO: startListening " + Utility.ListeningCounter);
